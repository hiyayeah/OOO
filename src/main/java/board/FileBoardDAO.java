...
public class FileBoardDAO {
	...
	public int selectBoardListCount(String keyword, String select) {
		...
		try {
			...
			// => WHERE 절에 LIKE 결합
			String sql = "SELECT COUNT(*) FROM file_board WHERE " + select + " LIKE ?";
			pstmt = con.prepareStatement(sql);
	...
	public ArrayList<FileBoardDTO> selectBoardList(int startRow, int listLimit, String keyword, String select) {
		ArrayList<FileBoardDTO> boardList = null;
		...
		try {
			...
			String sql = "SELECT * FROM file_board WHERE " + select + " LIKE ? ORDER BY num DESC LIMIT ?,?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, "%" + keyword + "%"); // 검색어를 포함하기 위해 % 결합
	...
	// 최근 게시물 5개 목록 조회하는 selectRecentBoardList() 메서드 정의
		public ArrayList<FileBoardDTO> selectRencentBoardList() {
			ArrayList<FileBoardDTO> boardList = null;
			Connection con = null;
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			...
			try {
				...
				System.out.println("SQL 구문 오류 발생! - selectRencentlyBoardList()");
				e.printStackTrace();
			}
