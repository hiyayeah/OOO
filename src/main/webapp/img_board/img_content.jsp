...
<head>
<meta charset="UTF-8">
<title>img_board/img_content.jsp</title>
<style type="text/css">
table {
	border: solid 3px;
	text-align: center;
...
<script type="text/javascript">
	...
	function forwardReply() {
		...
		<%if(sId != null){ %>
			...
			location.href = "img_content_reply_writePro.jsp?boardType=img&num=<%=board.getNum()%>&name=<%=sId%>&content=" + content;
		<%} else {%>
			...
			<form action="#" method="post">
				<table class="table">
    				...
    				<tr style="border-bottom: dotted 1px;">
      					<th>내용</th>
    					<td colspan="3">
    						<img style="width: 500px" src="../img_upload/<%=board.getOriginal_file() %>">
					...
					%>
			    		<tr style="border-bottom: dotted 1px;">
			    			...
			    			<td>
			    				...
			    				onclick="location.href='img_content_reply_deletePro.jsp?num=<%=reply.getNum()%>&refNum=<%=num%>'">
			    				<%} %>
			    			</td>
			    		</tr>
		    			<%
				...
				<script>
					function func1(num) {
						...
						location.href="img_update.jsp?num=" + num;
						<%} else {%>
							alert("권한이 없습니다.");
						<% }%>
					}
					...
					function func2(num) {
						<%if(sId != null && sId.equals(board.getName())){ %>
						location.href="img_delete.jsp?num=" + num;
						<%} else {%>
							alert("권한이 없습니다.");
						<% }%>
					}
				...
				<div id="table_search" style="float: right;">
					...
					<input type="button" value="글목록" class="btn" onclick="location.href ='img.jsp'">
				</div>
			</form>
