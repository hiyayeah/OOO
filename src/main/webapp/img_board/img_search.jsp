...
ArrayList<ImgBoardDTO> boardList = dao.selectBoardList(startRow, listLimit, keyword, select);
...
<head>
<meta charset="UTF-8">
<title>img_board/img_search.jsp</title>
...
<body>
	...
	<article style="display: table; margin: 50px auto; width: 1400px;">
		...
		<!-- 검색 기능 -->
		<div id="table_search">
				<button type="button" style="float: left;" class="btn btn-light" onclick="location.href = 'img_write.jsp'">글쓰기</button>
		...
		<div style="width: 300px; margin-left: 50%; margin-right: auto;">
			...
			<%} else { %>
				<a style="text-decoration: none; font-size: 150%;" href="img.jsp?keyword=<%=keyword %>&pageNum=<%=pageNum - 1%>">&laquo;</a>
			<%} %>
			...
			<%for(int i = startPage; i <= endPage; i++) { %>
				...
				<%} else { %>
					<a style="text-decoration: none; font-size: 150%;" href="img.jsp?keyword=<%=keyword %>&pageNum=<%=i %>"><%=i %></a>
				<%} %>
			<%} %>
			...
			<%} else { %>
				<a style="text-decoration: none; font-size: 150%;" href="img.jsp?keyword=<%=keyword %>&pageNum=<%=pageNum + 1%>">&raquo;</a>
			<%} %>
