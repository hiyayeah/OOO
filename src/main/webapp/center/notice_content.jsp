			...
			<form action="#" method="post">
					...
					%>
			    		<tr style="border-bottom: dotted 1px;">
			    			...
			    			<td>
			    				...
			    				<input type="button" value="삭제" class="btn btn-primary btn-sm" onclick="location.href='notice_content_reply_deletePro.jsp?num=<%=reply.getNum()%>&refNum=<%=num%>'">
			    				<%} %>
			    			</td>
			    		</tr>
		    			<%
				...
				<script>
					function func1(num) {
						...
						location.href="notice_update.jsp?num=" + num;
						<%} else {%>
							alert("권한이 없습니다.");
						<% }%>
					}
					...
					function func2(num) {
						<%if(sId != null && sId.equals("admin")){ %>
						location.href="notice_delete.jsp?num=" + num;
						<%} else {%>
							alert("권한이 없습니다.");
						<% }%>
					}
